...
Once the runtime starts, you can access the project at [http://localhost:8080/logon](http://localhost:8080/logon).
...
```
./mvnw clean package
docker build -t logon:v1 .
```

You can then run the Docker image by executing:

```
docker run -it --rm -p 8080:8080 logon:v1
```
